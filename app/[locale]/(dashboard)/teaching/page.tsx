...
function LessonsTable({ teacherId }: { teacherId: string }) {
  ...
  const [selectedGradeByCurriculum, setSelectedGradeByCurriculum] = useState<
  ...
  const selectedAssignment = assignmentsWithProgress?.find(
    (a) => a._id === expandedCurriculum,
  );
  ...
  return (
    <div className="space-y-6">
      {assignmentsWithProgress.map((assignment) => {
        const totalProgress =
          assignment.progressSummary?.progressPercentage || 0;
        const isExpanded = expandedCurriculum === assignment._id;
        const lessons =
          isExpanded && assignmentLessonProgress
        ...
        return (
          <Card
            key={assignment._id}
            className="border-border/60 bg-card shadow-sm hover:shadow-md transition-shadow py-0"
          >
            <Collapsible
              open={isExpanded}
              onOpenChange={() =>
                setExpandedCurriculum(isExpanded ? null : assignment._id)
              }
            >
              ...
              <CollapsibleContent>
                <CardContent className="py-3 px-2 sm:py-5 sm:px-6">
                  <div className="border-t border-border/60 pt-2 sm:pt-6">
                    ...
                    {assignment.assignedGrades &&
                      assignment.assignedGrades.length > 1 && (
                        <Tabs
                          value={
                            selectedGradeByCurriculum[assignment._id] ||
                            assignment.assignedGrades[0]
                          }
                          onValueChange={(value) => {
                        ...
                        >
                          <TabsList>
                            {assignment.assignedGrades.map((gradeCode) => {
                              ...
                              const gradeName =
                                assignmentLessonProgress?.grades?.find(
                                  (g: {
                                    id: string;
                                    name: string;
                                    code: string;
                    ...
                    <div className="grid grid-cols-4 gap-2 sm:gap-6 mb-3 sm:mb-6">
                      {[1, 2, 3, 4]
                        .slice(0, assignment.numberOfQuarters)
                        .map((quarterNum) => {
                          // Get selected grade
                          const selectedGrade =
                            selectedGradeByCurriculum[assignment._id] ||
                          ...
                          const lessonsInQuarter = lessons.filter(
                            (l: Record<string, unknown>) => {
                              ...
                              if (
                                ...
                                if (
                                  l.gradeCodes &&
                                  Array.isArray(l.gradeCodes) &&
                                  l.gradeCodes.length > 0
                                ) {
                                  return (l.gradeCodes as string[]).includes(
                                    selectedGrade,
                                  );
                                }
                                if (l.gradeCode) {
                          ...
                          lessonsInQuarter.forEach(
                            (l: Record<string, unknown>) => {
                              ...
                              if (
                                ...
                                const groupsForSelectedGrade =
                                  assignmentLessonProgress.assignedGroupCodes.filter(
                                    (groupCode: string) =>
                                      groupCode.startsWith(selectedGrade + "-"),
                                  );

                                // Count completed groups for this grade
                                const completedGroupsForGrade = Array.isArray(
                                  l.progressByGrade,
                                )
                                  ? l.progressByGrade.filter(
                                      (p: Record<string, unknown>) =>
                                        groupsForSelectedGrade.includes(
                                          (p.groupCode as string) || "",
                                        ) &&
                                ...
                                const lessonCompletionPercentage =
                                  groupsForSelectedGrade.length > 0
                                    ? (completedGroupsForGrade /
                                        groupsForSelectedGrade.length) *
                                      100
                                    : 0;
                                ...
                                totalCompletionScore +=
                                  typeof l.completionPercentage === "number"
                          ...
                          const isQuarterExpanded =
                            expandedQuarter === quarterKey;

                          return (
                            <Card
                              key={quarterNum}
                              className={`cursor-pointer py-0 border-border/60 shadow-sm hover:shadow-md transition-all ${
                                isQuarterExpanded
                            ...
                            >
                              <CardHeader className="py-2 px-2 sm:p-4">
                                ...
                                />
                                <CardDescription
                                  className={`text-center text-[9px] sm:text-xs ${
                                    isQuarterExpanded
                                      ? "text-white/90"
                    ...
                    {[1, 2, 3, 4]
                      ...
                      .map((quarterNum) => {
                        ...
                        const lessonsByQuarter = lessons.filter(
                          (l: Record<string, unknown>) => {
                            if (l.quarter !== quarterNum) return false;
                        ...
                        return (
                          <Card
                            key={quarterNum}
                            className="bg-accent/20  py-0 shadow-sm "
                          >
                                                  ...
                                                  <div className="flex items-center gap-1 flex-wrap">
                                                    {(() => {
                                                      ...
                                                      const groupsForSelectedGrade =
                                                        assignmentLessonProgress.assignedGroupCodes.filter(
                                                          (groupCode: string) =>
                                                            groupCode.startsWith(
                                                              selectedGrade +
                                                                "-",
                                                            ),
                                                        );

                                                      return groupsForSelectedGrade.map(
                                                        (groupCode: string) => {
                                                          ...
                                                          const hasEvidence =
                                                            lessonData.progressByGrade?.find(
                                                              (
                                                                p: Record<
                                                                  string,
                                                                  unknown
                                          ...
                                          <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-4 pl-8 sm:pl-0">
                                                ...
                                                {(() => {
                                                  if (
                                                    ...
                                                    return lessonData.progress
                                                      ?.evidencePhotoStorageId ||
                                                      lessonData.progress
                                                        ?.evidenceDocumentStorageId
                                                      ? "Evidence uploaded"
                                                      : "No evidence";
                                                  }
                                                  ...
                                                  const evidenceCount =
                                                    Array.isArray(
                                                      lessonData.progressByGrade,
                                                    )
                                                      ? lessonData.progressByGrade.filter(
                                                          (
                                                            p: Record<
                                                              string,
                                                              unknown
                                            ...
                                            <TeacherDialog
                                              ...
                                              trigger={
                                                <Button
                                                  size="sm"
                                                  className="bg-deep-koamaru hover:bg-deep-koamaru/90 h-7 sm:h-9 text-[10px] sm:text-sm px-2 sm:px-3"
                                                >
