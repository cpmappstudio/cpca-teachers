...
interface AddTeachersDialogMockProps {
    campusId: Id<"campuses">
}

export function AddTeachersDialogMock({ campusId }: AddTeachersDialogMockProps) {
    ...
    const trigger = (
        <Button className="bg-sidebar-accent h-9 dark:text-white gap-2">
            ...
            <span className="hidden md:inline">Add Teacher (Mock)</span>
        </Button>
    )
                    ...
                    <div className="bg-muted/30 rounded-lg p-4">
                        <div className="flex items-center gap-2 text-sm">
                            ...
                            <span className="font-medium">
                                Ready to add {selectedTeachers.length} teacher{selectedTeachers.length === 1 ? '' : 's'} to this campus
                            </span>
                        </div>
                        <div className="mt-2 text-xs text-muted-foreground">
                            Selected: {selectedTeachers.map(t => t.fullName).join(", ")}
                        </div>
                    </div>
                )}
            </div>
